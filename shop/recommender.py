...
class Recommender:
    ...
    def suggested_products_for(self, products, max_results=6):
        ...
        if len(products) == 1:
            ...
            #Сгенерируем временный ключ
            flat_ids = ''.join([str(id) for id in product_ids])
            tmp_key = f'tmp_{flat_ids}'
            #объединяем баллы всех товаров
            #сохраняем в полученное сорт. мн-во во временном ключе
        ...
        #получтаем предлагаемые товары и
        #сортируем их по порядку их появления
